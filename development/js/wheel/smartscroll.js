(function ($) { // eslint-disable-line func-names
  ...
  var MOUSE_EVENTS_STRING = '  DOMMouseScroll wheel MozMousePixelScroll';
      ...
      $(window).bind(MOUSE_EVENTS_STRING, function (e) { // eslint-disable-line func-names
        ...
        if (
          ...
          if (sectionIndexAtWindowTop !== sectionIndexAtWindowBottom
            ...
            //e.preventDefault();
            document.addEventListener('MOUSE_EVENTS_STRING', { passive: false });
            e.stopPropagation();
            if (scrollAction) {
              if (scrollAction === 'up') {
                if (options.toptotop) {
        ...
        if (sectionWrapperIsVisible()) {
          ...
          if (sectionIndexAtWindowTop !== sectionIndexAtWindowBottom
            ...
            switch (e.which) {
              ...
              case 38:
                //e.preventDefault();
                document.addEventListener(e, { passive: false });
                e.stopPropagation();
                if (options.toptotop) {
                  scrollToPixel(
                    sections[sectionIndexAtWindowMiddle - 2] + 1
